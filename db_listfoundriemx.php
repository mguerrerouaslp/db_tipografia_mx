...
// Consulta SQL para obtener los datos de la tabla fonts, ordenados por la columna 'typo' y agrupados por fundidoras
$consulta = "SELECT fonts.*, foundries.name AS foundry_name
              FROM fonts
              LEFT JOIN foundries ON fonts.foundry_id = foundries.id
              WHERE foundries.is_mx = 1
              AND fonts.foundry_id IS NOT NULL
              ORDER BY foundries.name, fonts.typo";
            ...
            <div class="desktop-12 tablet-12 nested columns">
                <div class="listado">
                    <div class="box-content" style="margin-left: 20px;">
                      <h4>Listado de fundidoras en México</h4>
                        <table>
                            <thead>
                                <tr>
                        ...
                        <?php echo "Mostrando $total_registros fuentes tipográficas asociadas a fundidoras mexicanas"; ?>
                    </div><!-- // .box-content -->
                </div><!-- // .listado -->
